# Entrée-sortie (I/O) des données géographiques {#read-write}
...
Les packets suivant sont nécesaire pour ce chapitre:
...
Le theme de ce chapitre est la lecture et l'écriture de données géographiques. 
Savoir*Importer* ou lire des données géographiques est essentiel pour la géocomputation\index{geocomputation}: les applications modernes sont impossible sans données.
La production (*sortie*) de données est également essentielle, elle permet à d'autres personnes d'utiliser ou enrichir les jeux de données fruit de votre travail. 
Rassemble, ces processus sont communément només entrée/sortie (E/S) ou en anglais *imput/output* (I/O).
Ils sont souvent mis en place hativement début et à la fin des projets, puis ignorées.
Cependant, l'importation et l'exportation de données sont fondamentales pour la réussite ou l'échec de projets : les petites erreurs d'E/S commises au début des projets (par exemple, l'utilisation d'un jeu de données obsolète) peuvent entraîner de gros problèmes par la suite.

Il existe de nombreux formats de fichiers géographiques, chacun ayant ses propres avantages et inconvénients, comme décrit dans la section  \@ref(file-formats).
La lecture et l'écriture depuis et vers ces formats sont traitées respectivement dans les sections \@ref(data-input) et \@ref(data-output).
En ce qui concerne la recherche de données, la section \@ref(retrieving-data) décrit les *géoportails* et comment les mobiliser pour la collecte de données.
Des paquets spécialisés facilitant l'importation de données géographiques, à partir de sources telles qu'OpenStreetMap, sont décrits dans la section \ref(geographic-data-packages).
Si vous souhaitez intégrer vos données dans les services web (ou si vous voulez être sûr que vos données respectent les normes établies), les métadonnées géographiques sont importantes et décrites dans la section \@ref(geographic-metadata).
Une autre façon d'obtenir des données spatiales est d'utiliser des services web \@ref(geographic-web-services).
La dernière section \@ref(visual-outputs) présente des méthodes de sauvegarde des sorties visuelles (cartes), en préparation du chapitre \@ref(adv-map) sur la visualisation des données.

## Formats de fichiers

\index{file formats}
Les jeux de données géographiques sont généralement stockés sous forme de fichiers ou dans des bases de données spatiales.
Les données vectorielles ou rasters ont des formats de fichier différents, tandis que les bases de données spatiales telles que [PostGIS] (https://postgis.net/) peuvent stocker les deux (voir également la section \@ref(postgis)).
Aujourd'hui, la variété des formats de fichiers peut sembler déconcertante, mais il y a eu beaucoup de consolidation et de normalisation depuis les débuts des logiciels SIG dans les années 1960, lorsque le premier programme largement distribué ([SYMAP](https://news.harvard.edu/gazette/story/2011/10/the-invention-of-gis/)) pour l'analyse spatiale a été créé à l'université de Harvard [@coppock_history_1991].

\index{GDAL}
GDAL (qui devrait se prononcer "goo-dal", le double "o" faisant référence à la programation orientée objet), la Geospatial Data Abstraction Library, a résolu de nombreux problèmes liés à l'incompatibilité entre les formats de fichiers géographiques depuis sa publication en 2000.
GDAL fournit une interface unifiée et performante pour la lecture et l'écriture de nombreux formats de données matricielles et vectorielles.^[Comme nous l'avons mentionné au chapitre \@ref(geometry-operations), GDAL contient également un ensemble de fonctions utilitaires permettant le mosaïquage, le rééchantillonnage, le recadrage et la reprojection de données rasters, etc.]
De nombreux programmes SIG libres et propriétaires, dont GRASS GIS, ArcGIS\index{ArcGIS} et QGIS\index{QGIS}, utilisent GDAL\index{GDAL} derrière leur GUI\index{interface utilisateur graphique} pour effectuer le travail d'ingestion et de restitution des données géographiques dans les formats appropriés.

GDAL\index{GDAL} permet d'accéder à plus de 200 formats de données vectorielles et rasters.
La table \@ref(tab:formats) présente quelques informations de base sur les formats de fichiers spatiaux sélectionnés et souvent utilisés.

```{r formats, echo=FALSE}
file_formats = tibble::tribble(~Name, ~Extension, ~Info, ~Type, ~Model, 
                         "ESRI Shapefile", ".shp (le fichier principal)", "Format populaire composé d'au moins trois fichiers. Pas de prise en charge pour : les fichiers > 2GB ; les types mixtes ; les noms > 10 caractères ; les colonnes > 255.", "Vecteur", "Partiellement ouvert",
                         "GeoJSON", ".geojson", "Étend le format d'échange JSON en incluant un sous-ensemble de la représentation *simple features* ; principalement utilisé pour stocker des coordonnées en longitude et latitude ; il est étendu par le format TopoJSON.", "Vecteur", "Ouvert",
                         "KML", ".kml", "Format basé sur XML pour la visualisation spatiale, développé pour être utilisé avec Google Earth. Le fichier KML zippé forme le format KMZ.", "Vecteur", "Ouvert",
                         "GPX", ".gpx", "Schéma XML créé pour l'échange de données GPS.", "Vecteur", "Ouvert",
                         "FlatGeobuf", ".fgb", "Format de fichier unique permettant la lecture et l'écriture rapides de données vectorielles. Possède des capacités de diffusion en continu.", "Vecteur", "Ouvert",
                         "GeoTIFF", ".tif/.tiff", "Format raster populaire. Fichier TIFF contenant des métadonnées spatiales supplémentaires.", "Raster", "Ouvert",
                         "Arc ASCII", ".asc", "Format textuel dans lequel les six premières lignes représentent l'en-tête du raster, suivies des valeurs des cellules  disposées en lignes et en colonnes.", "Raster", "Ouvert",
                         "SQLite/SpatiaLite", ".sqlite", "Standalone  relational database, SpatiaLite is the spatial extension of SQLite.", "Vector and raster", "Ouvert",
                         "ESRI FileGDB", ".gdb", "Objets spatiaux et non spatiaux créés par ArcGIS. Permet : classes d'entités multiples ; topologie. Support limité de GDAL.", "Vecteur and raster", "Propriétaire",
                         "GeoPackage", ".gpkg", "Base de données légère basée sur SQLite permettant un échange de géodonnées facile et indépendant de la plate-forme", "Vecteur et raster(très limité)", "Ouvert"
                         )
knitr::kable(file_formats, 
             caption = "Formats de fichiers spatiaux choisis.",
             caption.short = "Formats de fichiers spatiaux choisis.",
             booktabs = TRUE) |> 
...
\index{Shapefile}
\index{GeoPackage}
a création de l'Open Geospatial Consortium ([OGC] (https://www.ogc.org/)) en 1994 a constitué une étape importante dans la normalisation et l'ouverture des formats de fichiers.
Au-delà de la définition du modèle de données des *simple features* (voir section \@ref(intro-sf)), l'OGC coordonne également le développement de normes ouvertes, utilisées par exemple dans des formats de fichiers tels que GML\index{GML}, KML\index{KML} et GeoPackage\index{GeoPackage}.
Les formats de fichiers ouverts tels que ceux approuvés par l'OGC présentent plusieurs avantages par rapport aux formats propriétaires : les normes sont publiées, garantissent la transparence et offrent aux utilisateurs la possibilité de développer et d'adapter les formats de fichiers à leurs besoins spécifiques.

ESRI Shapefile est le format d'échange de données vectorielles le plus répandu, mais il ne s'agit pas d'un format ouvert (bien que sa spécification le soit).
Il a été développé au début des années 1990 et présente un certain nombre de limitations.
Tout d'abord, il s'agit d'un format multi-fichier, qui se compose d'au moins trois fichiers.
Il ne prend en charge que 255 colonnes, les noms de colonnes sont limités à dix caractères et la taille du fichier est limitée à 2 Go.
En outre, ESRI Shapefile ne prend pas en charge tous les types de géométrie possibles, par exemple, il n'est pas en mesure de faire la distinction entre un polygone et un multipolygone.^[Pour en savoir plus sur les limitations d'ESRI Shapefile et les formats de fichiers alternatifs possibles, visitez http://switchfromshapefile.org/].
Malgré ces limites, il manquait depuis longtemps une alternative viable. 
Ces dernières années, [GeoPackage](https://www.geopackage.org/)\index{GeoPackage} est apparu et semble être un candidat plus qu'approprié pour remplacer ESRI Shapefile.
Geopackage est un format d'échange d'informations géospatiales et un standard de l'OGC. 
La norme GeoPackage décrit les règles de stockage des informations géospatiales dans un minuscule conteneur SQLite.
GeoPackage est donc un conteneur de base de données spatiales léger, qui permet de stocker des données vectorielles et rasters, mais aussi des données non spatiales et des extensions.
Outre GeoPackage, il existe d'autres formats d'échange de données géospatiales qui méritent d'être examinés (tableau \ref(tab:formats)).
